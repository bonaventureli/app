#!/bin/bash
APPNAME=${0}.app
FROMNAME=001/ember-firmware/Utilities/UpgradeFPDriver
case ${1} in
    "make")
	rm -rf $APPNAME
	mkdir -p $APPNAME/Utilities/UpgradeFPDriver
	mkdir -p $APPNAME/C++/include
	mkdir -p $APPNAME/C++/include/rapidjson
	cp -r $FROMNAME/* $APPNAME/Utilities/UpgradeFPDriver
...
#	cp -r 001/ember-firmware/C++/Logger.cpp $APPNAME/C++
	...
	cd $APPNAME/Utilities/UpgradeFPDriver
	make
    ;;
    "clean")
	rm -rf $APPNAME
